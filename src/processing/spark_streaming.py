...
class SparkStreamingProcessor:
    ...
    def parse_and_transform(self, kafka_df):
        ...
        final_df = kafka_df.select(
            ...
            get_json_object(json_col, "$.P")
            .cast(DoubleType())
            .alias("price_change_percent"),
            get_json_object(json_col, "$.v").cast(DoubleType()).alias("volume"),
            col("partition").alias("kafka_partition"),
            col("offset").alias("kafka_offset"),
            (get_json_object(json_col, "$.E").cast(LongType()) / 1000)
            .cast("timestamp")
            .alias("processed_at"),
        ).dropDuplicates(
            ["symbol", "processed_at"]
        )  # Idempotency: loại bỏ duplicate
    ...
    def write_to_postgres(self, df, epoch_id):
        ...
        if row_count > 0:
            try:
                ...
                df_to_write.write.format("jdbc").option("url", postgres_url).option(
                    "dbtable", "real_time_prices"
