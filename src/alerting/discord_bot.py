...
async def price(interaction: discord.Interaction, coin: str = "BTC"):
    try:
        ...
        # Handle NULL values
        current_price = (
            latest["current_price"] if pd.notna(latest["current_price"]) else 0
        )
        price_change = (
            latest["price_change_percent"]
            if pd.notna(latest["price_change_percent"])
            else 0
        )
...
async def summary(interaction: discord.Interaction):
    try:
        ...
        df["price_change_percent"] = pd.to_numeric(
            df["price_change_percent"], errors="coerce"
        ).fillna(0)
        ...
        embed = discord.Embed(
            title="📊 Daily Market Summary", color=discord.Color.gold()
        )
...
async def help_cmd(interaction: discord.Interaction):
    ...
    embed.add_field(
        name="Coins",
        value="BTC, ETH, BNB, SOL, XRP, ADA, DOGE, DOT, AVAX, MATIC, LINK, LTC, UNI, ATOM, ETC",
        inline=False,
    )
...
async def daily_report():
    ...
    try:
        ...
        embed.add_field(
            name="🚀 Top 5 Gainers", value=gainers_text or "N/A", inline=False
        )
        ...
        embed.add_field(
            name="📉 Top 5 Losers", value=losers_text or "N/A", inline=False
        )
