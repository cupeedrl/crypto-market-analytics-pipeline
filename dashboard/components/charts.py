...
def render_correlation_heatmap(df_history):
    ...
    st.markdown(
        '<h2 class="section-title">Market Correlation</h2>', unsafe_allow_html=True
    )
    ...
    try:
        ...
        st.write(
            f"**Data stats:** {daily_prices['symbol'].nunique()} coins, {daily_prices['date'].nunique()} days"
        )

        # Get average price per day per symbol
        daily_avg = (
            daily_prices.groupby(["date", "symbol"])["current_price"]
            .mean()
            .reset_index()
        )
        ...
        pivot_df = daily_avg.pivot(
            index="date", columns="symbol", values="current_price"
        )
...
def render_risk_analytics(df_history):
    ...
    st.markdown('<h2 class="section-title">Risk Analytics</h2>', unsafe_allow_html=True)

    try:

        volatility_df = AnalyticsService.calculate_volatility_metrics(df_history)

        if volatility_df.empty:
            st.warning("Not enough data.")
            return

        for col in ["avg_return", "volatility", "sharpe_ratio"]:
            volatility_df[col] = (
                volatility_df[col].replace([np.inf, -np.inf], 0).fillna(0)
            )

        volatility_df["sharpe_ratio_abs"] = volatility_df["sharpe_ratio"].abs()
        ...
        with col2:
            ...
            sharpe = volatility_df.sort_values("sharpe_ratio", ascending=False)
...
def render_anomaly_detection(df_history):
    ...
    st.markdown(
        '<h2 class="section-title">Anomaly Detection</h2>', unsafe_allow_html=True
    )
    ...
    try:
        ...
        anomalies_df = AnalyticsService.detect_anomalies(
            df_history, threshold=threshold
        )

        if anomalies_df.empty:
            st.success(f"No anomalies detected (threshold={threshold})")
            return

        col1, col2 = st.columns(2)

        col1.metric("Anomalies", len(anomalies_df))

        col2.metric("Affected Coins", anomalies_df["symbol"].nunique())

        display_df = anomalies_df[["symbol", "date", "z_score"]].copy()

        display_df["z_score"] = display_df["z_score"].round(2)

        st.dataframe(
            display_df.sort_values("date", ascending=False), use_container_width=True
        )
