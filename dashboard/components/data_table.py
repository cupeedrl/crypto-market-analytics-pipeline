...
def render_data_table(df_latest):
    ...
    # Filter data
    if search_term:
        df_filtered = df_latest[df_latest['symbol'].str.contains(search_term, case=False, na=False)]
    else:
        df_filtered = df_latest
    
    # Display table
    st.dataframe(
        df_filtered[['symbol', 'current_price', 'price_change_percent', 'volume', 'processed_at']],
