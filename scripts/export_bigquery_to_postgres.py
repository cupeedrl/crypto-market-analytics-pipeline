...
def export_bigquery_to_postgres():
    ...
    features_df = features_df.sort_values(["symbol", "date_hour"]).reset_index(
        drop=True
    )

    # Tính toán các chỉ số
    features_df["daily_return"] = (
        features_df.groupby("symbol")["close_price"].pct_change() * 100
    )
    features_df["volatility_7d"] = features_df.groupby("symbol")[
        "daily_return"
    ].transform(lambda x: x.rolling(7).std())
    features_df["volatility_30d"] = features_df.groupby("symbol")[
        "daily_return"
    ].transform(lambda x: x.rolling(30).std())
    features_df["volume_change_pct"] = (
        features_df.groupby("symbol")["total_volume"].pct_change() * 100
    )
    features_df["rsi_14"] = 50.0  # Tạm thời fix 50
    features_df["price_to_volume_ratio"] = (
        features_df["close_price"] / features_df["total_volume"]
    )
    ...
    features_df["trend_strength"] = features_df.groupby("symbol")[
        "close_price"
    ].transform(lambda x: x.rolling(7).apply(calc_slope, raw=False))
